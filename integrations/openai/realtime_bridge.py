        ...
        "parameters": {
            ...
            "properties": {
                "render_type": {
                    ...
                    "description": (
                        "data_board=tableau/liste structuree, "
                        "chart=graphique/courbe/barres/donut, "
                        "kpi_cards=indicateurs/metrics/KPIs/chiffres cles, "
                        "timeline=chronologie/planning/dates/jalons/echeances, "
                        "roadmap=plan par phases/feuille de route, "
                        "comparison=comparaison/versus/cote a cote, "
                        "document_draft=courrier/lettre/redaction/compte-rendu/CR, "
                        "document_insight=analyse document/PDF/fichier, "
                        "action_board=checklist/actions/taches a faire/liste actions/avant lancement/todo, "
                        "kanban_board=kanban/sprint/backlog/taches par colonnes/avancement/En cours/A faire/Termine, "
                        "contact_board=fiche contact/qui est, "
                        "map_board=carte/adresse/itineraire, "
                        "decision_board=aide a la decision/pour ou contre/options, "
                        "budget_board=budget/depenses/couts/finances, "
                        "meeting_board=reunion/participants/ordre du jour/decisions, "
                        "media_board=fichiers/photos/images, "
                        "form_board=formulaire/saisie, "
                        "context_panel=contexte general/explication, "
                        "missing_info=infos manquantes, "
                        "status_rail=etat services/systeme/sante"
                    )
                },
                "payload": {
                    "type": "object",
                    "description": (
                        ...
                        "context_panel: {title, sections:[{heading,body}], summary?}. "
                        "missing_info: {question?, fields:[str], suggestions?:[str]}. "
                        "status_rail: {title?, services:[{name,status:'active'|'warn'|'error',detail?}], summary?}. "
                        "chart: {title, type:'line'|'bar'|'donut', labels:[str], datasets:[{label,data:[number]}], summary?}. "
                        "kpi_cards: {kpis:[{label,value,trend?:'up'|'down'|'flat',change?}], summary?}. "
                        "timeline: {events:[{date,title,description?}], summary?}. "
                        "roadmap: {phases:[{title,description?,status?:'done'|'active'|'pending'}], summary?}. "
                        "comparison: {title_left, title_right, rows:[{label,left,right,winner?:'left'|'right'}], summary?}. "
                        "document_insight: {title, boxes:[{title,body}], tags?:[{label,type:'ok'|'warn'|'info'|'error'}], actions?:[{label}]}. "
                        "decision_board: {options:[{name,pros:[str],cons:[str],recommended?:bool}], recommendation?}. "
                        ...
                        "contact_board: {contact:{name,phone?,email?,company?,role?,initials?,trust_level?}, history?:[{type,preview,date?}]}. "
                        "map_board: {address, label?, duration_walk_min?, duration_car_min?, distance_km?}. "
                        "budget_board: {kpis:[{label,value,alert?:bool}], categories:[{name,amount,percent}], summary?}. "
                        "media_board: {files:[{name,type:'pdf'|'image'|'spreadsheet'|'doc',size?}], summary?}. "
                        "form_board: {title, description?, fields:[{label,type?:'text'|'radio',options?:[str],required?:bool,placeholder?,prefill?}]}."
                    ),
