...
gcloud run deploy "$SERVICE" \
  ...
  --region="$REGION" \
  --project="$PROJECT" \
  --update-env-vars="PROPRIO_PASSWORD=${PROPRIO_PASSWORD},PROPRIO_EMAIL=${PROPRIO_EMAIL},REDIS_URL=${REDIS_URL},SIMLI_API_KEY=${SIMLI_API_KEY},SIMLI_FACE_ID=${SIMLI_FACE_ID},CARTESIA_API_KEY=${CARTESIA_API_KEY},ELEVENLABS_API_KEY=${ELEVENLABS_API_KEY},LUNA_MODE=${LUNA_MODE},SENTRY_DSN=${SENTRY_DSN},ENVIRONMENT=cloudrun,OPENAI_VOICE_NAME=${OPENAI_VOICE_NAME:-coral}" \
  "$@"
